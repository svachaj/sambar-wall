...
func (s *CoursesService) GetCoursesList() ([]types.CourseType, error) {
	...
	for i, courseType := range courses {
		...
		GROUP BY tc.ID, tct.Name1 , tct.Description1 , tc.TimeFrom ,tc.TimeTo , tcd.Name1 , tcag.Name1, tc.Capacity, tct.Code , tcd.Code
		ORDER BY tct.Code , tct.Name1 , tcd.Code, tc.TimeFrom ;
		`, courseType.ID)
